...
class BudgetAIService {
    ...
    public function hasEnoughHistoricalData(?int $categoryId, int $userId, int $month, int $year): bool {
        $monthsWithData = 0;

        // Check the 3 months before the target month/year for any spending data.
        for ($i = 1; $i <= 3; $i++) {
            ...
            if ($query->sum('amount') > 0) {
                $monthsWithData++;
            }
        }

        // Allow recommendation button when we have at least 2 months of historical totals.
        return $monthsWithData >= 2;
    }
}