<div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 border-l-4 border-purple-500">
                ...
                <div class="text-3xl font-bold text-gray-800 dark:text-gray-100">${{ number_format($totalSpent, 0) }}</div>
                @if($monthlyBudget > 0)
                    <div class="mt-2 text-sm {{ $totalSpent > $monthlyBudget ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400' }}">
                        {{ $totalSpent > $monthlyBudget ? 'Over' : 'Under' }} budget by ${{ number_format(abs($monthlyBudget - $totalSpent), 0) }}
                    </div>
                @endif
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 border-l-4 border-blue-500">
                ...
                <div class="text-3xl font-bold text-gray-800 dark:text-gray-100">${{ number_format($monthlyBudget, 0) }}</div>
                @if($monthlyBudget > 0)
                    <div class="mt-3">
                        <div class="flex items-center justify-between text-xs text-gray-600 dark:text-gray-300 mb-1">
                            <span>{{ $percentageUsed }}% used</span>
                            <span>${{ number_format($monthlyBudget - $totalSpent, 0) }} left</span>
                        </div>
                        <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                            <div
                                @class([
                                    'h-2 rounded-full',
                                    'bg-red-500' => $percentageUsed > 100,
                                    'bg-yellow-500' => $percentageUsed <= 100 && $percentageUsed > 80,
                                    'bg-green-500' => $percentageUsed <= 80,
                                    'w-full' => $percentageUsed >= 100,
                                    'w-11/12' => $percentageUsed >= 90 && $percentageUsed < 100,
                                    'w-10/12' => $percentageUsed >= 80 && $percentageUsed < 90,
                                    'w-8/12' => $percentageUsed >= 60 && $percentageUsed < 80,
                                    'w-6/12' => $percentageUsed >= 40 && $percentageUsed < 60,
                                    'w-4/12' => $percentageUsed >= 20 && $percentageUsed < 40,
                                    'w-2/12' => $percentageUsed > 0 && $percentageUsed < 20,
                                    'w-0' => $percentageUsed <= 0,
                                ])
                            ></div>
                        </div>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 border-l-4 border-green-500">
                ...
                <div class="text-3xl font-bold text-gray-800 dark:text-gray-100">{{ $expenseByCategory->count() }}</div>
                <div class="mt-2 text-sm text-gray-600 dark:text-gray-300">Active spending categories</div>
            </div>
            <!-- Recurring Card -->
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 border-l-4 border-orange-500">
                ...
                <div class="text-3xl font-bold text-gray-800 dark:text-gray-100">{{ $recurringExpenseCount }}</div>
                <div class="mt-2 text-sm text-gray-600 dark:text-gray-300">Active subscriptions</div>
            </div>
        </div>
        <!-- Charts Row -->
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
                <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-4">6-Month Spending Trend</h3>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <!-- Top Categories -->
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
                <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-4">Top Spending Categories</h3>
                <div class="space-y-4">
                    @forelse($topCategories as $category)
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-lg flex items-center justify-center bg-gray-100 dark:bg-gray-700">
                                    <svg class="w-3 h-3" viewBox="0 0 12 12" fill="none" aria-hidden="true">
                                        <circle cx="6" cy="6" r="5" fill="{{ $category->color }}" />
                                    </svg>
                                </div>
                                <div>
                            ...
                            <div class="text-right">
                                <div class="font-bold text-gray-800 dark:text-gray-100">${{ number_format($category->total, 0) }}</div>
                            </div>
                        </div>
                    @empty
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-100">Recent Expenses</h3>
                    <a href="/expenses" class="text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 text-sm font-medium">View All</a>
                </div>
                <div class="space-y-3">
                        ...
                        <div class="flex items-center justify-between py-3 border-b border-gray-100 dark:border-gray-700 last:border-0">
                            <div class="flex items-center gap-3">
                                @if($expense->category)
                                    <div class="w-10 h-10 rounded-lg flex items-center justify-center bg-gray-100 dark:bg-gray-700">
                                        <svg class="w-3 h-3" viewBox="0 0 12 12" fill="none" aria-hidden="true">
                                            <circle cx="6" cy="6" r="5" fill="{{ $expense->category->color }}" />
                                        </svg>
                                    </div>
                                @endif
                                <div>
                                    <div class="font-medium text-gray-800 dark:text-gray-100">{{ $expense->title }}</div>
                                    <div class="text-sm text-gray-500 dark:text-gray-400">{{ $expense->date->format('M d, Y') }}</div>
                                </div>
                            </div>
                            <div class="text-right">
                                <div class="font-bold text-gray-800 dark:text-gray-100">-${{ number_format($expense->amount, 0) }}</div>
    ...
    <script id="dashboard-chart-data" type="application/json">
        {!! json_encode([
            'monthlyLabels' => $monthlyComparison->pluck('month')->values(),
            'monthlyTotals' => $monthlyComparison->pluck('total')->values(),
            'categoryLabels' => $expenseByCategory->pluck('name')->values(),
            'categoryTotals' => $expenseByCategory->pluck('total')->values(),
